...
public sealed class WebDashboardServer : IDisposable
{
    ...
    public void Start()
    {
        ...
        var ip = GetLanIPv4() ?? "localhost";
        SetAccessUrl($"http://{ip}:{Port}/");
            ...
            for (var attempt = 1; !token.IsCancellationRequested; attempt++)
            {
                ...
                try
                {
                    ...
                    while (!token.IsCancellationRequested && !process.HasExited && DateTime.UtcNow < deadline)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(current) && current.Contains("trycloudflare.com", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            while (!token.IsCancellationRequested && !process.HasExited)
                            {
                                ...
                                // Registrar periódicamente que el proceso sigue vivo.
                                if (DateTime.Now.Second % 15 == 0)
                                    WriteCloudflareStatus("CONECTADO", $"URL pública: {current}; cloudflared PID={process.Id}");
                            }
                            break;
                        }
                    ...
                    if (process.HasExited)
                    {
                        ...
                        WriteCloudflareStatus("DESCONECTADO", $"cloudflared terminó con código {process.ExitCode}");
                    }
                ...
                if (!token.IsCancellationRequested)
                {
                    var waitSeconds = Math.Min(15, 2 + Math.Max(0, attempt - 1));
    ...
    private static void TryCaptureTunnelUrlStatic(string line)
    {
        ...
        var match = Regex.Match(line,
            @"https://[A-Za-z0-9][A-Za-z0-9-]*\.trycloudflare\.com(?:/[^\s\)\]\|]*)?",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        if (!match.Success) return;

        var url = match.Value.TrimEnd('.', ',', ';', ')', ']', '|', '\\', '"', '\'');
        if (!url.EndsWith("/", StringComparison.Ordinal)) url += "/";
        if (!url.Contains(".trycloudflare.com", StringComparison.OrdinalIgnoreCase)) return;
    ...
    private static string GetMobileBaseUrl()
    {
        ...
        if (!string.IsNullOrWhiteSpace(publicUrl) && publicUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase))
            return publicUrl;
    ...
    private static string BuildMobilePairingJson()
    {
        ...
        return JsonSerializer.Serialize(new
        {
            ...
            mode = (Current?.AccessUrl?.StartsWith("http", StringComparison.OrdinalIgnoreCase) == true) ? "PUBLICO" : "LAN"
        });
    }
    ...
    {
        var json = BuildMobilePairingJson();
        return "FPM2." + Convert.ToBase64String(Encoding.UTF8.GetBytes(json))
            .Replace("+", "-").Replace("/", "_").TrimEnd('=');
    }
...
ORDER BY p.description LIMIT 500;";
    ...
    private static async Task HandleMobileAsync(TcpClient client, NetworkStream stream, string request, string target, CancellationToken token)
    {
        ...
        try
        {
            ...
            if(productMatch.Success)
            {
                ...
                if(method=="POST" || method=="PUT")
                {
                    ...
                    if(method=="POST")
                    {
                        ...
                        using var cmd=cn.CreateCommand();cmd.Transaction=tx;cmd.CommandText="INSERT INTO products(barcode,description,sale_price,wholesale_price,cost_price,stock,min_stock,category,unit,is_bulk,active,uses_inventory,updated_at) VALUES($b,$d,$s,$w,$c,$st,$m,$cat,$u,$bulk,1,$inv,CURRENT_TIMESTAMP);SELECT last_insert_rowid();";cmd.Parameters.AddWithValue("$b",dto.Barcode.Trim());cmd.Parameters.AddWithValue("$d",dto.Description.Trim());cmd.Parameters.AddWithValue("$s",dto.SalePrice);cmd.Parameters.AddWithValue("$w",dto.WholesalePrice);cmd.Parameters.AddWithValue("$c",dto.CostPrice);cmd.Parameters.AddWithValue("$st",dto.Stock);cmd.Parameters.AddWithValue("$m",dto.MinStock);cmd.Parameters.AddWithValue("$cat",dto.Category?.Trim()??"");cmd.Parameters.AddWithValue("$u",string.IsNullOrWhiteSpace(dto.Unit)?"UN":dto.Unit.Trim());cmd.Parameters.AddWithValue("$bulk",dto.Bulk?1:0);cmd.Parameters.AddWithValue("$inv",dto.UsesInventory?1:0);id=Convert.ToInt32(cmd.ExecuteScalar());
                    ...
                    {
                        using var cmd=cn.CreateCommand();cmd.Transaction=tx;cmd.CommandText="UPDATE products SET barcode=$b,description=$d,sale_price=$s,wholesale_price=$w,cost_price=$c,stock=$st,min_stock=$m,category=$cat,unit=$u,is_bulk=$bulk,uses_inventory=$inv,updated_at=CURRENT_TIMESTAMP WHERE id=$id AND active=1";cmd.Parameters.AddWithValue("$id",id);cmd.Parameters.AddWithValue("$b",dto.Barcode.Trim());cmd.Parameters.AddWithValue("$d",dto.Description.Trim());cmd.Parameters.AddWithValue("$s",dto.SalePrice);cmd.Parameters.AddWithValue("$w",dto.WholesalePrice);cmd.Parameters.AddWithValue("$c",dto.CostPrice);cmd.Parameters.AddWithValue("$st",dto.Stock);cmd.Parameters.AddWithValue("$m",dto.MinStock);cmd.Parameters.AddWithValue("$cat",dto.Category?.Trim()??"");cmd.Parameters.AddWithValue("$u",string.IsNullOrWhiteSpace(dto.Unit)?"UN":dto.Unit.Trim());cmd.Parameters.AddWithValue("$bulk",dto.Bulk?1:0);cmd.Parameters.AddWithValue("$inv",dto.UsesInventory?1:0);if(cmd.ExecuteNonQuery()!=1)throw new InvalidOperationException("Producto no encontrado o inactivo.");
                    }
            ...
            if(purchaseMatch.Success && method=="DELETE")
            {
                ...
                if(!string.Equals(status,"DRAFT",StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("Solo se pueden eliminar órdenes que todavía están en borrador.");
            ...
            {var body=await ReadRequestBodyAsync(stream,request,token);var dto=JsonSerializer.Deserialize<MobilePurchaseDto>(body,new JsonSerializerOptions{PropertyNameCaseInsensitive=true})??throw new InvalidOperationException("Compra inválida.");if(dto.SupplierId<=0||dto.Items==null||dto.Items.Count==0)throw new InvalidOperationException("Proveedor y productos son obligatorios.");using var cn=Database.Open();using var tx=cn.BeginTransaction();var total=dto.Items.Sum(x=>x.Quantity*x.UnitCost);var orderNo="OC-"+DateTime.Now.ToString("yyyyMMddHHmmss");using(var c=cn.CreateCommand()){c.Transaction=tx;c.CommandText="INSERT INTO purchase_orders(order_no,supplier_id,status,order_date,total,notes,created_by) VALUES($no,$s,'DRAFT',CURRENT_TIMESTAMP,$t,$n,$u);SELECT last_insert_rowid();";c.Parameters.AddWithValue("$no",orderNo);c.Parameters.AddWithValue("$s",dto.SupplierId);c.Parameters.AddWithValue("$t",total);c.Parameters.AddWithValue("$n",dto.Notes??"");c.Parameters.AddWithValue("$u",MobileUserId());var id=Convert.ToInt64(c.ExecuteScalar());foreach(var x in dto.Items){using var q=cn.CreateCommand();q.Transaction=tx;q.CommandText="INSERT INTO purchase_order_items(order_id,product_id,quantity,unit_cost,received_quantity,notes) VALUES($o,$p,$q,$c,0,$n)";q.Parameters.AddWithValue("$o",id);q.Parameters.AddWithValue("$p",x.ProductId);q.Parameters.AddWithValue("$q",x.Quantity);q.Parameters.AddWithValue("$c",x.UnitCost);q.Parameters.AddWithValue("$n",x.Notes??"");q.ExecuteNonQuery();}tx.Commit();AuditService.Log(MobileUserId(),"MOBILE_PURCHASE_CREATE","COMPRAS",$"Orden {orderNo} · Total ${total:N2}");await JsonAsync(stream,new{ok=true,id,orderNo,total},token);return;}}
    ...
    private sealed class MobileSupplierDto { public string Name {get;set;}=""; public string? Document {get;set;} public string? Phone {get;set;} public string? Email {get;set;} public string? Address {get;set;} }
    ...
    private sealed class MobileProductDto { public string Barcode {get;set;}=""; public string Description {get;set;}=""; public double SalePrice {get;set;} public double WholesalePrice {get;set;} public double CostPrice {get;set;} public double Stock {get;set;} public double MinStock {get;set;} public string? Category {get;set;} public string? Unit {get;set;} public bool Bulk {get;set;} public bool UsesInventory {get;set;}=true; }
    ...
    private async Task HandleClientAsync(TcpClient client, CancellationToken token)
    {
        ...
        if (parts.Length < 2 || !(string.Equals(parts[0], "GET", StringComparison.OrdinalIgnoreCase) || string.Equals(parts[0], "POST", StringComparison.OrdinalIgnoreCase) || string.Equals(parts[0], "PUT", StringComparison.OrdinalIgnoreCase)))
        {
    ...
    private static async Task JsonAsync(NetworkStream stream, object value, CancellationToken token, int status = 200)
        => await SendAsync(stream, status, "application/json; charset=utf-8", JsonSerializer.Serialize(value), token);
...
COALESCE((SELECT SUM(amount) FROM cash_movements WHERE session_id=$s AND movement_type='SALE' AND payment_method='EFECTIVO'),0);";
    ...
    private static List<object> GetTodaySales()
    {
        ...
        using (var cmd=cn.CreateCommand())
        {
            cmd.CommandText = @"SELECT s.id,s.ticket_no,s.total,s.payment_method,s.sale_channel,s.created_at,
...
SELECT c.id,c.name,c.document,c.phone,c.email,c.credit_limit,
       ...
       (SELECT p4.created_at FROM customer_accounts p4 WHERE p4.customer_id=c.id AND p4.entry_type='PAYMENT' ORDER BY p4.id DESC LIMIT 1) AS ultimo_abono
FROM customers c
...
LIMIT 150;";
    ...
    {
        using var cn=OpenReadOnly(); using var cmd=cn.CreateCommand();
        cmd.CommandText=@"SELECT cs.id,cs.status,cs.opening_amount,cs.opened_at,cs.closed_at,cs.closing_amount,cs.expected_amount,cs.difference,COALESCE(u.full_name,''),COALESCE(cs.mercado_pago_enabled,0),COALESCE(cs.mercado_pago_opening_amount,0),COALESCE(cs.mercado_pago_expected_amount,0) FROM cash_sessions cs LEFT JOIN users u ON u.id=cs.user_id WHERE cs.status='OPEN' ORDER BY cs.id DESC LIMIT 1;";
        using var r=cmd.ExecuteReader();
    ...
    {
        var s=GetOpenSessionId(); using var cn=OpenReadOnly(); using var cmd=cn.CreateCommand();
        cmd.CommandText=@"SELECT cm.created_at,cm.movement_type,cm.payment_method,cm.concept,cm.amount,COALESCE(u.full_name,''),COALESCE(cm.voided,0),COALESCE(cm.void_reason,''),COALESCE(v.full_name,v.username,'')
...
ORDER BY cm.id DESC;";
        cmd.Parameters.AddWithValue("$s",s); using var r=cmd.ExecuteReader(); var list=new List<object>();
        while(r.Read()) list.Add(new {fecha=Text(r,0),tipo=Text(r,1),medio=Text(r,2),concepto=Text(r,3),importe=Dec(r,4),usuario=Text(r,5),anulado=r.GetInt32(6)!=0,motivoAnulacion=Text(r,7),anulo=Text(r,8)}); return list;
    }
