...
""";
    ...
    {
        EnsureInitialized();
        if (!_state.Activated || string.IsNullOrWhiteSpace(_state.LicenseId)) return false;
        var baseUrl = ReadLicenseServerUrl();
        if (string.IsNullOrWhiteSpace(baseUrl)) return false;

        try
        {
            using var client = new HttpClient { Timeout = TimeSpan.FromSeconds(4) };
            var url = baseUrl.TrimEnd('/') + "/api/license/status/" + Uri.EscapeDataString(_state.LicenseId) + "?machineId=" + Uri.EscapeDataString(_state.MachineId);
            ...
            if (root.TryGetProperty("revoked", out var revoked) && revoked.ValueKind == JsonValueKind.True)
            {
                ...
                Save(); SaveRegistry(); SyncDatabase();
                return true;
            }
            if (root.TryGetProperty("expiresAtUtc", out var exp) && exp.ValueKind == JsonValueKind.String && DateTime.TryParse(exp.GetString(), out var serverExpiry))
            {
                var utc = serverExpiry.ToUniversalTime();
                if (utc < _state.ExpiresAtUtc) { _state.ExpiresAtUtc = utc; Save(); SaveRegistry(); SyncDatabase(); }
            }
        }
        catch { /* Sin Internet o servidor no disponible: no bloquear una licencia válida. */ }
        return false;
    }
    ...
    public static void StartOnlineMonitor()
    {
        ...
        _onlineTimer = new System.Threading.Timer(_ =>
        {
            try
            {
