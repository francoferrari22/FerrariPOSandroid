...
public static class ProductService
{
    ...
    public static void Delete(int id)
    {
        using var cn = Database.Open();
        using var cmd = cn.CreateCommand();
        cmd.CommandText = "UPDATE products SET active=0,updated_at=CURRENT_TIMESTAMP WHERE id=$id";
        cmd.Parameters.AddWithValue("$id", id);
        cmd.ExecuteNonQuery();
        AuditService.Log(Session.UserId, "PRODUCT_DELETE", "INVENTARIO", $"Producto ID {id}");
    }
}
