...
public class MainForm : Form
{
    ...
    private void Build()
    {
        ...
        {
            if (Session.IsAdmin) ShowOwnedDialog(new MobilePairingForm());
            else MessageBox.Show("Solo un administrador puede mostrar el QR de vinculación.", "FerrariPOS Manager", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }, 110);
    ...
    private void ImportMobileOpenTickets()
    {
        ...
        try
        {
            ...
            var emptyMobileTableIds = new List<int>();
            while (r.Read())
            {
                ...
                {
                    // PRODUCTO EN COMÚN es un registro especial. Versiones anteriores
                    // del Manager Android podían enviarlo con ProductId=0; nunca debe
                    // descartarse por intentar resolver ese 0 contra products.
                    if (line.IsCommon)
                    {
                        using var ccn = Database.Open();
                        using var cc = ccn.CreateCommand();
                        cc.CommandText = "SELECT id,description FROM products WHERE barcode='__COMUN__' LIMIT 1";
                        using var cr = cc.ExecuteReader();
                        if (cr.Read())
                        {
                            cart.Add(new CartItem
                            {
                                ProductId = cr.GetInt32(0),
                                Barcode = "COMÚN",
                                Description = string.IsNullOrWhiteSpace(line.Description) ? cr.GetString(1) : line.Description,
                ...
                // Un ticket móvil de mesa sin productos no debe quedar persistido ni volver a aparecer.
                // Lo marcamos para borrar después de cerrar el DataReader (SQLite no permite
                // modificar de forma segura la misma tabla mientras este reader está abierto).
                if (cart.Count == 0)
                {
                    emptyMobileTableIds.Add(tableId);
                    continue;
                }
                seen.Add(tableId);
            ...
            // Elimina de forma persistente las mesas móviles vacías detectadas arriba.
            // Así no reaparecen en el siguiente refresco automático.
            foreach (var emptyTableId in emptyMobileTableIds)
                WebDashboardServer.RemoveMobileOpenTicket(emptyTableId);
            ...
            var emptyMobilePendingIds = new List<long>();
            ...
            using (var pcmd = pcn.CreateCommand())
            {
                ...
                while (pr.Read())
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        if (line.IsCommon)
                        {
                            using var ccn = Database.Open(); using var cc = ccn.CreateCommand();
                            cc.CommandText = "SELECT id,description FROM products WHERE barcode='__COMUN__' LIMIT 1";
                            using var cr = cc.ExecuteReader();
                            if (cr.Read()) cart.Add(new CartItem { ProductId=cr.GetInt32(0), Barcode="COMÚN", Description=string.IsNullOrWhiteSpace(line.Description)?cr.GetString(1):line.Description, UnitPrice=line.UnitPrice, RetailPrice=line.UnitPrice, WholesalePrice=line.UnitPrice, CostPrice=0, Stock=0, IsBulk=false, UsesInventory=false, Quantity=line.Quantity, DiscountAmount=line.Discount, IsCommon=true });
                        }
                        else if (p2r.Read()) cart.Add(new CartItem { ProductId=line.ProductId, Barcode=p2r.GetString(0), Description=string.IsNullOrWhiteSpace(line.Description)?p2r.GetString(1):line.Description, UnitPrice=line.UnitPrice, RetailPrice=p2r.GetDouble(2), WholesalePrice=p2r.GetDouble(3), Stock=p2r.GetDouble(5), IsBulk=p2r.GetInt32(6)!=0, UsesInventory=p2r.GetInt32(7)!=0, Quantity=line.Quantity, DiscountAmount=line.Discount, IsCommon=false });
                    }
                    // Si la venta móvil pendiente no contiene productos válidos, se elimina
                    // automáticamente del almacenamiento para que no quede un ticket fantasma.
                    if (cart.Count == 0)
                    {
                        emptyMobilePendingIds.Add(pendingId);
                        continue;
                    }
            ...
            // Borra las ventas móviles vacías después de cerrar el DataReader.
            foreach (var emptyPendingId in emptyMobilePendingIds)
                WebDashboardServer.RemoveMobilePendingTicket(emptyPendingId);
