...
public class CashForm : Form
{
    ...
    private void GeneralDailyClosing()
    {
        try
        {
            if (!Session.IsAdmin)
            {
                MessageBox.Show("El CORTE Z GENERAL del día está reservado al ADMINISTRADOR porque consolida la información de todos los cajeros.", "Corte Z general", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
