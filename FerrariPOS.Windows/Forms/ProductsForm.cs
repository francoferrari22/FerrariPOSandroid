...
public class ProductsForm : Form
{
    ...
    private void Selected(object? s, EventArgs e)
    {
        if (grid.CurrentRow?.DataBoundItem is Product p)
        {
            ...
            min.Text = p.MinStock.ToString("N2"); category.Text = p.Category; unit.Text = p.Unit; bulk.Checked = p.IsBulk; noInventory.Checked = !p.UsesInventory; SelectSupplierForProduct(p.Id); noInventory.Enabled = InventoryControlService.IsGlobalEnabled;
    ...
    private void Clear()
    {
        ...
        profitPercent.SelectedIndex = 0; customProfit.Clear(); LoadSuppliers(0); addsIva21 = false; roundSaleTo5 = false; SetIvaButtonState(); SetRoundButtonState(); UpdateProfitLabel(); noInventory.Enabled = InventoryControlService.IsGlobalEnabled; barcode.Focus();
    }
    ...
    private void Delete()
    {
        ...
        {
            MessageBox.Show("La eliminación fue cancelada: el motivo es obligatorio.", "Eliminar producto", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
