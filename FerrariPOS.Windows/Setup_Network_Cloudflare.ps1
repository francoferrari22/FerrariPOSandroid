...
try {
    $ruleName = 'FerrariPOS servidor local 8787'
    if (-not (Get-NetFirewallRule -DisplayName $ruleName -ErrorAction SilentlyContinue)) {
        New-NetFirewallRule -DisplayName $ruleName -Direction Inbound -Action Allow -Protocol TCP -LocalPort 8787 -Profile Private -ErrorAction Stop | Out-Null
        Log "FIREWALL: regla privada TCP 8787 creada."
    } else { Log "FIREWALL: regla TCP 8787 ya existe." }
